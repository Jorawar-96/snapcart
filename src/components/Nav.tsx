'use client'
import { Boxes, ClipboardIcon, Cross, LogOut, Menu, Package, Plus, PlusCircle, Search, ShoppingCartIcon, User, X } from 'lucide-react'
...
function Nav({user}:{user:IUser}) {
  ...
  const sideBar = menuOpen?createPortal(
    <AnimatePresence>
      ...
      className='fixed top-0 h-full w-[75%] sm:w-[60%] z-9999 bg-linear-to-b from-green-400/20 to-green-900/90 backdrop-blur-xl border-r border-green-400/20 shadow-[0_0_50px_-10px_rgba(0,255,100,0.3)] flex flex-col p-6 text-white'
      >
        ...
        <div className='flex items-center gap-3 p-3 mt-3 rounded-xl bg-white/10 hover:bg-white/15 transition-all shadow-inner'>
          ...
          <div>
            ...
            <p className='text-xs text-green-100 capitalize tracking-role'>{user.role}</p>
          </div>
        </div>
        <div className='flex flex-col gap-2 font-medium mt-6'>
          <Link href={"/admin/add-grocery"} className='flex items-center gap-3 p-3 rounded-lg bg-white/10 hover:bg-white/20 hover:pl-4 transition-all'>  <PlusCircle className='w-5 h-5'/> Add Groceries</Link>
          ...
          <Link href={""} className='flex items-center gap-3 p-3 rounded-lg bg-white/10 hover:bg-white/20 hover:pl-4 transition-all'><ClipboardIcon className='w-5 h-5'/> Manage Orders</Link>     
        </div>
  ...
  return (
    <div className='w-[95%] fixed top-4 left-1/2 -translate-x-1/2 bg-gradient-to-r from-green-500 to-green-700 rounded-2xl shadow-lg shadow-black/30 flex justify-between items-center h-20 px-4 md:px-8 x-50 '>
      <Link href={"/"} className='text-white font-extrabold text-2x sm:text-3xl tracking-wide hover:scale-105 transition-transform'>
      Snapcart
      </Link>
      ...
      <div className='flex items-center gap-3 md:gap-6 relative'> 
        ...
        <div className='hidden md:flex gap-4'>
          <Link href={"admin/add-grocery"} className='flex  items-center gap-2 bg-white text-green-700 font-semibold px-4 py-2 rounded-full hover:bg-green-100 transition'> <PlusCircle className='w-5 h-5'/> Add Groceries</Link>
          <Link href={""} className='flex  items-center gap-2 bg-white text-green-700 font-semibold px-4 py-2 rounded-full hover:bg-green-100 transition'><Boxes className='w-5 h-5'/> View Grocery</Link>          
          <Link href={""} className='flex  items-center gap-2 bg-white text-green-700 font-semibold px-4 py-2 rounded-full hover:bg-green-100 transition'><ClipboardIcon className='w-5 h-5'/> Manage Orders</Link>          
        </div>       
